...
def artificiel_occlusion(x, y, l_infos):
    nbr_s = x.shape[0]
    x_l, y_l, tags_oc = [], [], []
    l_infos_ext = []
    for i in range(nbr_s):
        s_resh = x[i, :].reshape(50, 50)
        info = l_infos[i]
        y_ = y[i, :]
        # add occlusions: left, right, down, up, noise (middle).
        for part in [None, "left", "right", "up", "down", "middle"]:
            x_l.append(hide_part(copy.deepcopy(s_resh), part).reshape(50*50))
            y_l.append(y_)
            l_infos_ext.append(info)
            tags_oc.append(part)
    x_l = np.asarray(x_l)
    y_l = np.asarray(y_l)
    return x_l.astype(type(x)), y_l.astype(type(y)), l_infos_ext, tags_oc
